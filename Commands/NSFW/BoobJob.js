...
module.exports = {
    name: "boobjob",
    usage: ["Le dicen \"La Rusa\"... ```{prefix}boobjob <@usuario>```"],
    enabled: true,
    aliases: [],
    category: "NSFW",
    memberPermissions: [],
    ...
    async execute(client, message, args, data){
        try{
              const img = await nekoapi.rolplay_nsfw.boobjob()
      let mention = message.mentions.members.first()
    if(!mention) {
      message.channel.send("Debes mencionar a una persona para... bueno, eso")
    } else {
      if(mention.user.id === message.author.id) {
        message.channel.send("No creo que sea posible hacerte eso a ti mismo")
      }
      ...
      if(mention.user.id !== message.author.id && mention.user.id !== "637421092264476675") {
        ...
        .setDescription(`**${message.author.username}** le hace la rusa a **${mention.user.username}**`)
        .setColor("RANDOM")
        .setImage(img.url);
