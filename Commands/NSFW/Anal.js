...
module.exports = {
    name: "anal",
    usage: ["Por el maletero... ```{prefix}anal <@usuario>```"],
    enabled: true,
    aliases: [],
    category: "NSFW",
    memberPermissions: [],
    ...
    async execute(client, message, args, data){
        try{
              const img = await nekoapi.rolplay_nsfw.anal()
      let mention = message.mentions.members.first()
    if(!mention) {
      message.channel.send("Debes mencionar a una persona para... bueno, eso")
    } else {
      if(mention.user.id === message.author.id) {
        message.channel.send("No creo que la tengas tan grande para poder hacer eso")
      }
      ...
      if(mention.user.id !== message.author.id && mention.user.id !== "637421092264476675") {
        ...
        .setDescription(`**${message.author.username}** se ha follado a **${mention.user.username}**, pero por el culo`)
        .setColor("RANDOM")
        .setImage(img.url);
